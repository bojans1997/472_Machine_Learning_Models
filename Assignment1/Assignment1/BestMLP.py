...
#base mlp vesion clf = MLPClassifier(100,'logistic','sgd',0.0001,'auto','constant', 0.001, 0.5, 200, True, None, 0.0001, False, False, 0.9, True, False, 0.1, 0.9, 0.999, 1e-08, 10, 15000)
mlp = MLPClassifier(max_iter=100)

parameter_space = {
    'hidden_layer_sizes': [(35,35,35), (100,75,50)],
...
np.savetxt("BEST-MLP-DS1.csv", np.transpose([index_array, prediction_result]), delimiter=",", fmt="%2.2f")
...
with open("BEST-MLP-DS1.csv", "ab") as f:
    f.write(b"\nConfusion Matrix")
    f.write(b"\n")
...
bestParamsSet1 = clf.best_params_
...
np.savetxt("BEST-MLP-DS2.csv", np.transpose([index_array, prediction_result]), delimiter=",", fmt="%2.2f")
...
with open("BEST-MLP-DS2.csv", "ab") as f:
    f.write(b"\nConfusion Matrix")
    f.write(b"\n")
    np.savetxt(f, conf_matrix, delimiter=",", fmt="%2.2f")
    f.write(b"\nClass,Precision,Recall,F1-Measure")
...
bestParamsSet2 = clf.best_params_

print("\n\nBest parameters for first data set:\n ", bestParamsSet1)
print("\n\nBest parameters for second data set:\n ", bestParamsSet2)