...
np.savetxt("Base-DT-DS1.csv", np.transpose([index_array, prediction_result]), delimiter=",", fmt="%2.2f")
...
with open("Base-DT-DS1.csv", "ab") as f:
    f.write(b"\nConfusion Matrix")
    f.write(b"\n")
...
np.savetxt("Base-DT-DS2.csv", np.transpose([index_array, prediction_result]), delimiter=",", fmt="%2.2f")
...
with open("Base-DT-DS2.csv", "ab") as f:
    f.write(b"\nConfusion Matrix")
    f.write(b"\n")
    np.savetxt(f, conf_matrix, delimiter=",", fmt="%2.2f")
    f.write(b"\nClass,Precision,Recall,F1-Measure")
